                                       ...
                                       if($row_notification['SoLuong'] >= 0){
                                          ...
                                          while($row_notification = $stmt_notification->fetch(PDO::FETCH_ASSOC)){
                                             if($row_notification['Status'] == 0){
                                                echo '
                                                <a href="update_notification.php?ID_Noti='.$row_notification['ID_Noti'].'&admin-product=true" class="iq-sub-card notification-items notification-unseen">
                                                ...
                                                echo '
                                                <a href="update_notification.php?ID_Noti='.$row_notification['ID_Noti'].'&admin-product=true" class="iq-sub-card notification-items">
                                                   <div class="media align-items-center">
                                                      <div class="media-body ml-3">
                                                         <h6 class="mb-0 ">'.$row_notification['Mota'].'</h6>
                                                      </div>
                              ...
                              <table class="data-tables table table-striped table-bordered" style="width:100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="width: 18%;">Thông tin</th>
                                ...
                                <tbody>
                                    <?php
                                       while ($res_sp = $stmt_sp->fetch(PDO::FETCH_ASSOC)) {
                                          ?>
                                             <tr>
                                                <td><?php echo $res_sp['ID_SanPham']?></td>
                                                <td><img class="img-fluid rounded" src="<?php
                                                   $id_img = $res_sp['ID_SanPham'];
                                                   $sql_img = "select * from hinhanhsanpham where ID_SanPham = '$id_img'";
                                                   $stmt_img = $conn->prepare($sql_img);
                                                   $query_img = $stmt_img->execute();
                                                   $res_img = $stmt_img->fetch(PDO::FETCH_ASSOC);
                                                   echo $res_img['link'];
                                                ?>" alt=""></td>
                                                <td><?php echo $res_sp['TenSP']?></td>
                                                <td>
                                                   <?php
                                                      $id_loai = $res_sp['ID_LoaiSanPham'];
                                                      $sql_loai = "select * from loaisanpham where ID_LoaiSanPham = '$id_loai'";
                                                      $stmt_loai = $conn->prepare($sql_loai);
                                                      $query_loai = $stmt_loai->execute();
                                                      $res_loai = $stmt_loai->fetch(PDO::FETCH_ASSOC);
                                                      echo $res_loai['TenLoai'];
                                                   ?>
                                                </td>
                                                <td>
                                                   <?php
                                                      ...
                                                      $stmt_hang = $conn->prepare($sql_hang);
                                                      $query_hang = $stmt_hang->execute();
                                                      $res_hang = $stmt_hang->fetch(PDO::FETCH_ASSOC);
                                                      echo $res_hang['TenHang'];
                                                   ?>
                                                </td>
                                                <td>
                                                   <p class="mb-0"><?php echo $res_sp['ThongTin']?></p>
                                                </td>
                                                <td><?php echo number_format($res_sp['Gia'], 0, ',', '.');?>đ</td>                                     
                                                <td>
                                                   <div class="flex align-items-center list-user-action">
                                                      <a class="bg-primary" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit" href="admin-edit-product.php?ID_SanPham=<?php echo $res_sp['ID_SanPham']?>"><i class="ri-pencil-line"></i></a>
                                                      <a class="bg-primary" data-toggle="tooltip" data-placement="top" title="" data-original-title="Xoá" href="admin-delete-product.php?ID_SanPham=<?php echo $res_sp['ID_SanPham']?>"><i class="ri-delete-bin-line"></i></a>
                                                   </div>
                                                </td>
                                             </tr>
                                          <?php
