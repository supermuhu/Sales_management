<?php
   ...
   $sql_getinfo_hang = "select * from loaisanpham where ID_LoaiSanPham = '$ID_Hang'";
   ...
   if (!empty($_POST['submit'])) {
    if (isset($_POST['tenhang'])) {
        ...
        $sql_add_hang = "update loaisanpham set TenLoai = '$tenhang' where ID_LoaiSanPham = '$ID_Hang'";
        $stmt_add_hang = $conn->prepare($sql_add_hang);
        $query_add_hang = $stmt_add_hang->execute();
        if ($query_add_hang) {
            header("location:admin-edit-loaisp.php?ID_Hang=$ID_Hang");
        }
    }
}
?>
...
<html lang="en">
   <head>
      ...
      <title>Thay đổi thông tin loại sản phẩm</title>
      <!-- Favicon -->
                                       ...
                                       if($row_notification['SoLuong'] >= 0){
                                          ...
                                          while($row_notification = $stmt_notification->fetch(PDO::FETCH_ASSOC)){
                                             if($row_notification['Status'] == 0){
                                                echo '
                                                <a href="update_notification.php?ID_Noti='.$row_notification['ID_Noti'].'&admin-edit-loaisp='.$ID_Hang.'" class="iq-sub-card notification-items notification-unseen">
                                                ...
                                                echo '
                                                <a href="update_notification.php?ID_Noti='.$row_notification['ID_Noti'].'&admin-edit-loaisp='.$ID_Hang.'" class="iq-sub-card notification-items">
                                                   <div class="media align-items-center">
                                                      <div class="media-body ml-3">
                                                         <h6 class="mb-0 ">'.$row_notification['Mota'].'</h6>
                                                      </div>
                     ...
                     <div class="iq-card">
                        <div class="iq-card-header d-flex justify-content-between">
                           <div class="iq-header-title">
                              <h4 class="card-title">Thay đổi thông tin loại sản phẩm</h4>
                           </div>
                        </div>
                        <div class="iq-card-body">
                           <form action="" method="post" enctype="multipart/form-data">
                              <div class="form-group">
                                 ...
                                 <input type="text" class="form-control" name="tenhang" value="<?php echo $res_getinfo_hang['TenLoai']?>">
                              </div>
                              <button type="submit" class="btn btn-primary" name="submit" value="submit">Sửa</button>
                           </form>
                        </div>
                     </div>
                  </div>
