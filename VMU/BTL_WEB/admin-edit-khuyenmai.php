<?php
   ...
   if (!empty($_POST['submit'])) {
      ...
      $sql_add_khuyenmai = "UPDATE khuyenmai SET TenKhuyenMai = :TenKhuyenMai, MoTa = :MoTa, BatDau = :BatDau, KetThuc = :KetThuc, MucGiam = :MucGiam WHERE ID_KhuyenMai = :ID_KhuyenMai";
      $stmt_add_khuyenmai = $conn->prepare($sql_add_khuyenmai);
      $stmt_add_khuyenmai->bindParam(':TenKhuyenMai', $TenKhuyenMai);
      $stmt_add_khuyenmai->bindParam(':MoTa', $MoTa);
      $stmt_add_khuyenmai->bindParam(':BatDau', $BatDau);
      $stmt_add_khuyenmai->bindParam(':KetThuc', $KetThuc);
      $stmt_add_khuyenmai->bindParam(':MucGiam', $MucGiam);
      $stmt_add_khuyenmai->bindParam(':ID_KhuyenMai', $ID_KhuyenMai);
      $stmt_add_khuyenmai->execute();
   }
?>
<!doctype html>
<html lang="en">
   <head>
      ...
      <title>Sửa chương trình khuyến mại</title>
      <!-- Favicon -->
                                       ...
                                       if($row_notification['SoLuong'] >= 0){
                                          ...
                                          while($row_notification = $stmt_notification->fetch(PDO::FETCH_ASSOC)){
                                             if($row_notification['Status'] == 0){
                                                echo '
                                                <a href="update_notification.php?ID_Noti='.$row_notification['ID_Noti'].'&admin-edit-khuyenmai='.$ID_KhuyenMai.'" class="iq-sub-card notification-items notification-unseen">
                                                ...
                                                echo '
                                                <a href="update_notification.php?ID_Noti='.$row_notification['ID_Noti'].'&admin-edit-khuyenmai='.$ID_KhuyenMai.'" class="iq-sub-card notification-items">
                                                   <div class="media align-items-center">
                                                      <div class="media-body ml-3">
                                                         <h6 class="mb-0 ">'.$row_notification['Mota'].'</h6>
                                                      </div>
                     ...
                     <div class="iq-card">
                        <div class="iq-card-header d-flex justify-content-between">
                           <div class="iq-header-title">
                              <h4 class="card-title">Sửa chương trình khuyến mại</h4>
                           </div>
                        </div>
                        <div class="iq-card-body">
                           <?php
                              $sql_edit_khuyenmai = "SELECT * FROM khuyenmai WHERE ID_KhuyenMai = :ID_KhuyenMai";
                              $stmt_edit_khuyenmai = $conn->prepare($sql_edit_khuyenmai);
                              $stmt_edit_khuyenmai->bindParam(':ID_KhuyenMai', $ID_KhuyenMai);
                              $stmt_edit_khuyenmai->execute();
                              if($stmt_edit_khuyenmai->rowCount() > 0){
                                 $row_edit_khuyenmai = $stmt_edit_khuyenmai->fetch(PDO::FETCH_ASSOC);
                                 echo '
                                 <form id="khuyenmai-form" action="" method="post" enctype="multipart/form-data">
                                    <div class="form-group">
                                       <label>Tên chương trình khuyến mại:</label>
                                       <input value="'.$row_edit_khuyenmai['TenKhuyenMai'].'" required type="text" class="form-control" name="TenKhuyenMai">
                                    </div>
                                    <div class="form-group">
                                       <label>Mô tả chương trình khuyến mại:</label>
                                       <input value="'.$row_edit_khuyenmai['MoTa'].'" required type="text" class="form-control" name="MoTa">
                                    </div>
                                    <div class="form-group d-flex justify-content-between">
                                       <div style="flex-basis: 40%;">
                                          <label>Bắt đầu:</label>
                                          <input value="'.$row_edit_khuyenmai['BatDau'].'" required type="date" class="form-control start-end" name="BatDau" id="BatDau">
                                       </div>
                                       <div style="flex-basis: 40%;">
                                          <label>Kết thúc:</label>
                                          <input value="'.$row_edit_khuyenmai['KetThuc'].'" required type="date" class="form-control start-end" name="KetThuc" id="KetThuc">
                                       </div>
                                    </div>
                                    <div class="form-group">
                                       <label>Mức giảm:</label>
                                       <input value="'.$row_edit_khuyenmai['MucGiam'].'" required type="number" class="form-control" name="MucGiam" min="1" max="100">
                                    </div>
                                    <button type="submit" class="btn btn-primary" name="submit" value="submit" style="margin-top: 5px">Sửa</button>
                                 </form>
                                 ';
                              }
                           ?>
