                                    ...
                                    <?php
                                          ...
                                          while($row_notification = $stmt_notification->fetch(PDO::FETCH_ASSOC)){
                                             if($row_notification['Status'] == 0){
                                                echo '
                                                <a href="update_notification.php?ID_Noti='.$row_notification['ID_Noti'].'&checkout=true" class="iq-sub-card notification-items notification-unseen">
                                                ...
                                                echo '
                                                <a href="update_notification.php?ID_Noti='.$row_notification['ID_Noti'].'&checkout=true" class="iq-sub-card notification-items">
                                                   <div class="media align-items-center">
                                                      <div class="media-body ml-3">
                                                         <h6 class="mb-0 ">'.$row_notification['Mota'].'</h6>
                                                      </div>
                     ...
                     <div class="row align-item-center">
                           ...
                           <div class="iq-card">
                              ...
                              <div class="iq-card-body">
                                 <ul class="list-inline p-0 m-0">
                                    <?php
                                       $sql_giohang = "SELECT gh.ID_GioHang, gh.ID_Account, gh.SoLuong, sp.ID_SanPham, sp.TenSP, sp.Gia as GiaTruoc, COALESCE( CASE WHEN km.MucGiam IS NOT NULL THEN (sp.Gia * (100 - km.MucGiam)) / 100 ELSE sp.Gia END, sp.Gia ) AS Gia, COALESCE(km.MucGiam, 0) AS MucGiam, sp.Hang, sp.Status, sp.SoLuong as SoLuongKho, sp.ThongTin, sp.DaBan, sp.BaoHanh, sp.ID_KhuyenMai, sp.ID_LoaiSanPham, ha.link FROM giohang gh JOIN sanpham sp ON gh.ID_SanPham = sp.ID_SanPham LEFT JOIN khuyenmai km ON sp.ID_KhuyenMai = km.ID_KhuyenMai JOIN (SELECT ID_SanPham, MIN(ID_HASP) AS Min_ID_HASP FROM hinhanhsanpham GROUP BY ID_SanPham) AS ha_min ON sp.ID_SanPham = ha_min.ID_SanPham JOIN hinhanhsanpham ha ON ha_min.Min_ID_HASP = ha.ID_HASP WHERE gh.ID_Account = :ID_Account GROUP BY sp.ID_SanPham ORDER BY gh.ID_GioHang DESC";
                                       $stmt_giohang = $conn->prepare($sql_giohang);
                                       $stmt_giohang->bindParam(':ID_Account', $_SESSION['ID_Account']);
                                       $stmt_giohang->execute();
                                       $total = 0;
                                       if($stmt_giohang->rowCount() > 0){
                                          while($row_giohang = $stmt_giohang->fetch(PDO::FETCH_ASSOC)){
                                             if($row_giohang['Status'] == 1) $total += $row_giohang['Gia']*$row_giohang['SoLuong'];
                                             $ID_SanPham = $row_giohang['ID_SanPham'];
                                             echo '
                                             <li class="checkout-product">
                                                <div class="row align-items-center">
                                                   <div class="col-sm-2">
                                                      <a class="checkout-img-link" href="product_detail.php?ID='.$row_giohang['ID_SanPham'].'"><img class="img-fluid rounded" src="'.$row_giohang['link'].'" alt=""></a>
                                                   </div>
                                                   <div class="col-sm-5">
                                                      <div class="checkout-product-details">
                                                         <h5>'.$row_giohang['TenSP'].'</h5>
                                                         ';
                                                         if($row_giohang['Status'] == 0){
                                                            echo '<p class="text-red-cart margin-p">Hết hàng '.$row_giohang['SoLuong'].'/'.$row_giohang['SoLuongKho'].'</p>';
                                                         }else if($row_giohang['SoLuong'] == $row_giohang['SoLuongKho']){
                                                            echo '<p class="text-red-cart margin-p">Tối đa '.$row_giohang['SoLuong'].'/'.$row_giohang['SoLuongKho'].'</p>';
                                                         }else{
                                                            echo '<p class="text-success margin-p">Còn hàng '.$row_giohang['SoLuong'].'/'.$row_giohang['SoLuongKho'].'</p>';
                                                         }
                                                      echo'<div class="price">';
                                                      if($row_giohang['GiaTruoc'] != $row_giohang['Gia']){
                                                         echo '
                                                         <i class="discount-number">-'.$row_giohang['MucGiam'].'%</i>
                                                         <h5 class="discount-label-parent">'.number_format($row_giohang['GiaTruoc'], 0, ',', '.').'<div class="discount-label"></div></h5>
                                                         ';
                                                      }
                                                      echo '
                                                            <h5>'.number_format($row_giohang['Gia'], 0, ',', '.').'</h5>
                                                         </div>
                                                      </div>
                                                   </div>
                                                   <div class="col-sm-5">
                                                      <div class="row">
                                                         <div class="col-sm-10">
                                                            <div class="row align-items-center mt-2">
                                                               <div class="button_quantity col-sm-7 col-md-6">';
                                                      if($row_giohang['Status'] == 0 || $row_giohang['SoLuong'] == $row_giohang['SoLuongKho']){
                                                         echo '
                                                                  <button onclick="subToCart('.$ID_SanPham.')" class="fa fa-minus qty-btn" id="btn-minus"></button>
                                                                  <input readonly type="text" class="quantity" id="number_add_to_cart_'.$ID_SanPham.'" value="'.$row_giohang['SoLuong'].'" min="1">
                                                                  <button class="fa fa-plus qty-btn" id="btn-plus"></button>
                                                         ';
                                                      }else{
                                                         echo '
                                                                  <button onclick="subToCart('.$ID_SanPham.')" class="fa fa-minus qty-btn" id="btn-minus"></button>
                                                                  <input readonly type="text" class="quantity" id="number_add_to_cart_'.$ID_SanPham.'" value="'.$row_giohang['SoLuong'].'" min="1">
                                                                  <button onclick="addToCart('.$ID_SanPham.')" class="fa fa-plus qty-btn" id="btn-plus"></button>
                                                         ';
                                                      }
                                                      echo' 
                                                                  </div>
                                                               <div class="col-sm-5 col-md-6">
                                                                  <span class="product-price">'.number_format($row_giohang['Gia']*$row_giohang['SoLuong'], 0, ',', '.').' ₫</span>
                                                               </div>
                                                            </div>
                                                         </div>
                                                         <div class="col-sm-2">
                                                            <a href="delete_on_cart.php?ID_GioHang='.$row_giohang['ID_GioHang'].'&checkout=true" class="text-dark font-size-20"><i class="ri-delete-bin-7-fill"></i></a>
                                                         </div>
                                                      </div>
                                                   </div>
                                                </div>
                                    ...
                                    ?>
                                    <script>
                                    function addToCart(ID_SanPham) {
                                       // Lấy giá trị từ input
                                       const inputId = 'number_add_to_cart_' + ID_SanPham;
                                       const SoLuong = Number(document.getElementById(inputId).value) + 1;
                                       ...
                                       window.location.href = `add_to_cart_from_Checkout.php?ID_SanPham=${ID_SanPham}&SoLuong=${SoLuong}`;
                                    }
                                    function subToCart(ID_SanPham) {
                                       // Lấy giá trị từ input
                                       const inputId = 'number_add_to_cart_' + ID_SanPham;
                                       const SoLuong = Number(document.getElementById(inputId).value) - 1;
                        ...
                        <?php
                           if($total != 0){
                              echo '
                              <div class="col-lg-4">
                                 <div class="w-90-checkout iq-card">
                                    <div class="iq-card-body">
                                       <p><b>Chi tiết</b></p>
                                       ...
                                       <div class="d-flex justify-content-between">
                                          ...
                                          <span class="text-dark"><strong>'.number_format($total, 0, ',', '.').' đ</strong></span>
                                       </div>
                                       ...
                                       <script>
                                          ...
                                          document.getElementById('paymentButton').addEventListener('click', function(e) {
                                             ...
                                             if (paymentMethodSelected) {
                                                window.location.href = 'check-accept.php?ID_ThanhToan=' + paymentMethodSelected.id.split('_')[1];
                                             } else {
                           ...
                           <div class="w-90-checkout iq-card">
                              <div class="iq-card-body">
                                 ...
                                 <?php
                                    echo '
                                    <div class="d-flex justify-content-between">
                                       <span>Giá '.$SoLuongTrongGioHang.' sản phẩm</span>
