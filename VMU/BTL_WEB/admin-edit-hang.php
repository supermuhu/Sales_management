<?php
   ...
   //Truy vấn thông tin hãng
   $ID_Hang = $_GET['ID_Hang'];
   $sql_getinfo_hang = "select * from hang where ID_Hang = '$ID_Hang'";
   $stmt_getinfo_hang = $conn->prepare($sql_getinfo_hang);
   $query_getinfo_hang = $stmt_getinfo_hang->execute();
   $res_getinfo_hang = $stmt_getinfo_hang->fetch(PDO::FETCH_ASSOC);
        ...
        $link = "./asset/image/";
         if ($_SERVER["REQUEST_METHOD"] == "POST") {
            if(!empty($_FILES['avatar']['name'])){
               $file_name = $_FILES['avatar']['name']; // Lấy tên file từ thẻ input
               $file_tmp = $_FILES['avatar']['tmp_name'];
               $link = $link . strval($file_name);
               move_uploaded_file($file_tmp, $link);
            } else {
               $link = $res_getinfo_hang['link_hang'];
            }
         }
        $sql_add_hang = "update hang set TenHang = '$tenhang', link_hang = '$link' where ID_Hang = '$ID_Hang'";
        $stmt_add_hang = $conn->prepare($sql_add_hang);
        $query_add_hang = $stmt_add_hang->execute();
        if ($query_add_hang) {
            header("location:admin-edit-hang.php?ID_Hang=$ID_Hang");
        }
    }
}
?>
...
<html lang="en">
   <head>
      ...
      <title>Thay đổi thông tin hãng</title>
      <!-- Favicon -->
                                       ...
                                       if($row_notification['SoLuong'] >= 0){
                                          ...
                                          while($row_notification = $stmt_notification->fetch(PDO::FETCH_ASSOC)){
                                             if($row_notification['Status'] == 0){
                                                echo '
                                                <a href="update_notification.php?ID_Noti='.$row_notification['ID_Noti'].'&admin-edit-hang='.$ID_Hang.'" class="iq-sub-card notification-items notification-unseen">
                                                ...
                                                echo '
                                                <a href="update_notification.php?ID_Noti='.$row_notification['ID_Noti'].'&admin-edit-hang='.$ID_Hang.'" class="iq-sub-card notification-items">
                                                   <div class="media align-items-center">
                                                      <div class="media-body ml-3">
                                                         <h6 class="mb-0 ">'.$row_notification['Mota'].'</h6>
                                                      </div>
                     ...
                     <div class="iq-card">
                        <div class="iq-card-header d-flex justify-content-between">
                           <div class="iq-header-title">
                              <h4 class="card-title">Thay đổi thông tin hãng sản xuất</h4>
                           </div>
                        </div>
                        <div class="iq-card-body">
                           <form action="" method="post" enctype="multipart/form-data">
                              <div class="form-group">
                                 ...
                                 <input type="text" class="form-control" name="tenhang" value="<?php echo $res_getinfo_hang['TenHang']?>">
                              </div>
                              <div class="form-group">
                                 <label>Hình ảnh:</label>
                                 <div class="custom-file">
                                    <input type="file" name="avatar" id="imageUpload" class="custom-file-input" accept="image/png, image/jpeg">
                                    <img id="avatr" src="<?php echo $res_getinfo_hang['link_hang']?>" alt="" style="width: 350px; height: 300px; margin: 10px;">
                              ...
                              <button type="submit" class="btn btn-primary" name="submit" value="submit" style="margin-top: 150px">Sửa</button>
                           </form>
                        </div>
                     </div>
                  </div>
