...
<html lang="en">
   <head>
      ...
      <title>Các đơn hàng cần xác nhận - Shop 23h</title>
      <!-- Favicon -->
                                       ...
                                       if($row_notification['SoLuong'] >= 0){
                                          ...
                                          while($row_notification = $stmt_notification->fetch(PDO::FETCH_ASSOC)){
                                             if($row_notification['Status'] == 0){
                                                echo '
                                                <a href="update_notification.php?ID_Noti='.$row_notification['ID_Noti'].'&admin-accept=true" class="iq-sub-card notification-items notification-unseen">
                                                ...
                                                echo '
                                                <a href="update_notification.php?ID_Noti='.$row_notification['ID_Noti'].'&admin-accept=true" class="iq-sub-card notification-items">
                                                   <div class="media align-items-center">
                                                      <div class="media-body ml-3">
                                                         <h6 class="mb-0 ">'.$row_notification['Mota'].'</h6>
                                                      </div>
                     ...
                     <div class="iq-card">
                        <div class="iq-card-header d-flex justify-content-between">
                           <div class="iq-header-title">
                              <h4 class="card-title">Danh sách đơn chờ xác nhận</h4>
                           </div>
                        </div>
                        <?php
                           $selectedID = isset($_GET['ID_Account']) ? $_GET['ID_Account'] : null;

                           $sql_account_accept = "SELECT acc.ID_Account, us.HoTen FROM account acc JOIN userinfo us ON acc.ID_Account = us.ID_Account JOIN accept ac ON acc.ID_Account = ac.ID_Account GROUP BY ac.ID_Account";
                           $stmt_account_accept = $conn->prepare($sql_account_accept);
                           $stmt_account_accept->execute();

                           if($stmt_account_accept->rowCount() > 0){
                              echo '
                              ...
                              ';
                              while($row_account_accept = $stmt_account_accept->fetch(PDO::FETCH_ASSOC)){
                                 $selected = ($row_account_accept['ID_Account'] == $selectedID) ? ' checked' : '';
                                 echo '
                                       <li role="option">
                                          <input type="radio" id="account_'.$row_account_accept['ID_Account'].'" name="account" value="'.$row_account_accept['ID_Account'].'"'.$selected.' />
                                          <label for="account_'.$row_account_accept['ID_Account'].'"><i class="bx bxl-account_'.$row_account_accept['ID_Account'].'"></i>'.$row_account_accept['HoTen'].'</label>
                                       </li>
                                 ';
                              }
                              echo '
                              ...
                              optionsList.forEach((option) => {
                                 function handler(e) {
                                       ...
                                       if (e.type === "click" && e.clientX !== 0 && e.clientY !== 0) {
                                          ...
                                          // Redirect to admin-accept.php with ID_Account
                                          const accountID = this.querySelector('input[type="radio"]').value;
                                          window.location.href = 'admin-accept.php?ID_Account=' + accountID;
                                       }
                                       // Key Events (Enter)
                                       if (e.key === "Enter") {
                        ...
                        if($selectedID != NULL){
                           ...
                           $stmt_accept->bindParam(':ID_Account', $selectedID);
                           $stmt_accept->execute();
                           if($stmt_accept->rowCount() > 0){
                              while($row_accept = $stmt_accept->fetch(PDO::FETCH_ASSOC)){
                                 echo '
                                 <div class="order-container iq-cart-body" data-donhang-id="'.$row_accept['ID_DonHang'].'">
                                     <div class="STT_status">
                                         <h5>#'.$row_accept['ID_DonHang'].'</h5>
                                         <div class="accept-button-parent">
                                             <a class="bg-primary accept-button-link check-button" href="#" onclick="event.preventDefault(); checkOrder(\''.$row_accept['ID_DonHang'].'\', \''.$row_accept['ID_Account'].'\', \''.$row_accept['ID_ThanhToan'].'\');">
                                                 <i class="ri-check-fill"></i>
                                             </a>
                                             <a class="bg-primary accept-button-link" href="admin-reject-order.php?ID_DonHang='.$row_accept['ID_DonHang'].'&ID_Account='.$selectedID.'&reject=true">
